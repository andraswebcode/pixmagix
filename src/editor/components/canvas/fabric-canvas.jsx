...
class FabricCanvas extends Component {
	...
	componentDidMount(){
		...
		this._fabricCanvas = new Canvas('pixmagix_canvas');
	...
	shouldComponentUpdate(nextProps, nextState){
		...
		// Set loading images.
		if (!isEqual(loadingImages, this.props.loadingImages)){
			canvas.setLoadingImages(loadingImages);
		}
	...
	_updateFabricObjectProps(nextLayers = []){
		...
		nextLayers.forEach(props => {
			...
			const object = canvas.getObjectById(id);
			if (object){
				object.set(changedProps);
				// Also need to run setSrc function to update image.
				if ('src' in changedProps && object.setSrc){
					object.setSrc(changedProps.src);
				}
			}
		});
...
export default connect(state => ({
	...
	loadingImages:state.editor.loadingImages
}),{
	setEditor,
	addLayer,
	setLayerProps
