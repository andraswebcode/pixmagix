...
if ( function_exists( __NAMESPACE__ . '\\pixmagix_fs' ) ) {
    pixmagix_fs()->set_basename( false, __FILE__ );
} else {
...
if ( !function_exists( __NAMESPACE__ . '\\fail_php_version' ) ) {
    ...
    {
        $message = sprintf( esc_html__( 'PixMagix requires PHP version %s+, plugin is currently NOT RUNNING.', 'pixmagix' ), PIXMAGIX_REQUIRED_PHP_VERSION );
...
if ( !function_exists( __NAMESPACE__ . '\\fail_wp_version' ) ) {
    ...
    {
        $message = sprintf( esc_html__( 'PixMagix requires WordPress version %s+, plugin is currently NOT RUNNING.', 'pixmagix' ), PIXMAGIX_REQUIRED_WP_VERSION );
        $html_message = sprintf( '<div class="error">%s</div>', wpautop( $message ) );
        echo  wp_kses_post( $html_message ) ;
    }
